...
public class MinimumTimeToFinishAllJobs {
    ...
    public int minimumTimeRequired(int[] jobs, int k) {
        ...
        helper(jobs, workers, 0);
        return globalMin;
    }
    
    public void helper(int[] jobs, int[] worker, int current){
        if(current >= jobs.length){
            int val = Arrays.stream(worker).max().orElse(0);
            globalMin = Math.min(val, globalMin);
            return;
        }

        if(Arrays.stream(worker).max().getAsInt() >= globalMin)
            return;
            
        for(int i = 0 ; i < worker.length; i++){
            worker[i] += jobs[current];
            helper(jobs, worker, current + 1);
            worker[i] -= jobs[current];
            if(worker[i] == 0)
                break;
        }
