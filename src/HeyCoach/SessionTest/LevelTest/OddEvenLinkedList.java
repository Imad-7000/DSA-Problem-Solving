...
Given the head of a singly linked list, reorder the list such that all nodes at odd positions are grouped together followed by the nodes at even positions.
 The first node is considered at position 1 (odd), the second node at position 2 (even), and so on. It is essential that the relative order within the odd 
 and even groups is maintained as per the original list.
...
The output should be the elements of the reordered linked list, with each node's value separated by a space, in the same order as they appear 
in the final reordered list.
Example:
...
Explanation:
In the given linked list, the nodes at odd positions are [1, 3, 5], and the nodes at even positions are [2, 4].Reordering these nodes
 so that all odd nodes come before even nodes, while maintaining their relative order, gives us [1, 3, 5, 2, 4] as the result
 */
