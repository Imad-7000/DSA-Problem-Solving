...
public class StockSpanner {
    ...
    //One stack keeps track of larger elements, the second stack keeps track of indices
    Stack<Integer> index;
    Stack<Integer> stack;
    int count; // count is used to track current index
    public StockSpanner(){
        stack = new Stack<>();
        index = new Stack<>();
        count  = 0;
    ...
    public int next(int price) {
        ...
        if(stack.isEmpty())
            ...
            //if stack is empty, then no stock greater in past, hence span in count + 1 days (+1 to include todays day)
            if(stack.isEmpty())
                val = count + 1;
            else{
                val = count - index.peek();
            }
        }
        stack.push(price);
        index.push(count);
