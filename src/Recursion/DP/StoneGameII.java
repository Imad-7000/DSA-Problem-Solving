...
public class StoneGameII {
    public int stoneGameII(int[] piles) {
        return stoneGameII(piles, 2, 1, 0);
    }

    public int stoneGameII(int[] piles, int X, int M, int idx){
        if(idx >= piles.length)
            return 0;
        int anssss = 0;
        int max = -1;
        for(int i = idx; i < 2*M; i++){
            int ans = piles[i] + stoneGameII(piles, i + 1, Math.max(i + 1,M), i + 1);
