...
public class SmoothDescentPeriod {
    public long getDescentPeriods(int[] prices) {
        return smoothDesc(prices, prices.length - 1, prices[prices.length - 1] + 1);
    }

    public long smoothDesc(int[] prices, int i, int prev){
        if(i < 0 )
            return 0;
        long noPick = smoothDesc(prices, i - 1, prices[i]);
        long pick = 1;
        if(prices[i] + 1 == prev)
            pick += smoothDesc(prices, i - 1, prices[i]);
        return pick + noPick;
    }
}
